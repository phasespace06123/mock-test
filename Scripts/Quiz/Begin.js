...
function CheckResult() {
    ...
    $(".test-questions").find("li").each(function () {
        ...
        var CorrectAnsMarks = $("#" + a).find(".hdfCorrectAnsMarks").val();
        var InCorrectAnsMarks = $("#" + a).find(".hdfInCorrectAnsMarks").val();
...
$(document).ready(function () {
    ...
    if ($('#ExamID').val() == '14') {
        $('#drplanguage').append($("<option></option>").text('Gujarati').val('gujarati'));
    }
    ...
    $('.drplanguage').on('change', function (e) {
        ...
        $('.question-height > .MQ').each(function (index, item) {
            ...
            $.each(JSON.parse(PaperQuestions), function (i, item) {
                if (item.QuestionNo == currentQuestionNo && item.LangID == parseInt(newlang)) {
                    currentDisImg.attr('src', $('#datalink').val() + item.DescriptionImagePath);
